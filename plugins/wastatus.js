    ...
    Ktb.addCommand({pattern: 'snd$', fromMe: true, desc: 'Download status from wa'}, (async (message, match) => {    
        ...
        if (message.reply_message.video === false && message.reply_message.image) {
            ffmpeg(location)
                .save('status.png')
                .on('end', async () => {
                    await message.client.sendMessage(message.client.user.jid,fs.readFileSync('status.png'), MessageType.image ,{caption: '```WA STATUS GENERATED BY WHITEDEVIL```'  , mimetype: Mimetype.jpg, quoted: message.data,thumbnail: White.tm_b});
            });
        return 
        }
        ...
        ffmpeg(location)
            .save('status.mp4')
            .on('end', async () => {
                await message.client.sendMessage(message.client.user.jid,fs.readFileSync('status.mp4'), MessageType.video , {caption: '```WA STATUS GENERATED BY WHITEDEVIL```'  , mimetype: Mimetype.mp4, quoted: message.data,thumbnail: White.tm_b});
            });
        return 
    }));
